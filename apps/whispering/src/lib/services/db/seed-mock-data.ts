 ...
 */
export function generateMockRecording(options: {
	index: number;
	baseTimestamp: Date;
}): RecordingStoredInIndexedDB {
 ...
 */
export function generateMockTransformation(options: {
	index: number;
}): Transformation {
	const { index } = options;
 ...
 */
export function generateMockTransformationRun(options: {
	index: number;
	recordingIds: string[];
 ...
 */
export async function getIndexedDBCounts(): Promise<{
	recordings: number;
	transformations: number;
	runs: number;
}> {
